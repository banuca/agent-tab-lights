(function attachTabTitle(globalScope) {
  ...
  function createTitleRenderer(options) {
    ...
    // Leaves no trace: strips the prefix and removes the styling hooks, for
    // when the user turns the extension off on a tab that is already open.
    function clear() {
      const bare = cleanTitle();
      ...
      if (root?.dataset) {
        delete root.dataset.agentTabLightsState;
        delete root.dataset.agentTabLightsLabel;
      }
    }

    return Object.freeze({ render, clear, cleanTitle, describe });
  }
